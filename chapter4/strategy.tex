...
\section{Selection of Research Strategy} \label{sec:researchstrategy}
  ...
  This section will describe key aspects of the survey. The first part of this section is a review of human properties that may be interesting to include in this research. To ensure a data collection containing respondents with different human properties, we need to create a collection strategy. The last part of this section will include a description of the sample size required to be able to test the hypothesis. 
    ...
    \subsection{Sample Size} \label{sec:samplesize}
      ...
      %I innledningen her må du være mye mer spesifikk. Hvorfor mener du målrguppen er med en 1 mill? Det er åpenbart, men du må si det allikevel. 

      %Du må også argumentere for hvorfor en utvalgsstørrelse på 1000 er nok. Det holder ikke å si løst at det antagelig er det. 

      %Du skriver her også om utfordringer knyttet til at delgrupper er underrepresentert i utvalget. Når du gjør det må du også diskutere utfordringen med at dersom du påvirker utvalget, f.eks for å få inn flere venstrehendte, så påvirker du også resultatene dine - som du kan trekke basert på hele utvalget, fordi det ikke lenger representerer et tverrsnitt av befolkningen. 
    ...
    Since the sampling frame is wide, and the survey is going to be distributed with a self-selection sampling strategy over the Internet, it is still important to provide some level of control of the respondents. In the next section, we will look at some response strategy.
